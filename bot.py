...
try:
    ...
    # dotenv is optional; if not installed we'll just rely on environment variables
    pass
...
# Load bot token from environment first (recommended). If not set, fall back to
# the value in ./conf/settings.txt for backwards compatibility.
bot_token = os.getenv('BOT_TOKEN') or raw_config.get('bot_token')
if not bot_token or bot_token == 'YOUR_BOT_TOKEN':
    raise RuntimeError("Bot token not set. Set the BOT_TOKEN environment variable or update ./conf/settings.txt")
